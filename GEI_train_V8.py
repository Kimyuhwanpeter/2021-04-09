...
lr_scheduler = LinearDecay(FLAGS.lr, FLAGS.epochs * len_dataset, 15 * len_dataset)  # 이 부분은 꼭 다시 고치자!!
optim = tf.keras.optimizers.Adam(lr_scheduler, beta_1=0.5)
...
def cal_CDF(logits, labels, num_feature):
    ...
    logit_CDF = []
    label_CDF = []
    for i in range(FLAGS.batch_size):   # CDF 구하는것
        log = 0.
        lab = 0.
        logit = logits[i]
        logit = logit.numpy()
...
def cal_loss(model, images, age_labels, gener_labels, all_age_label, all_age_onehot):
    ...
    with tf.GradientTape() as tape:
        ...
        all_age_loss = (-tf.reduce_sum( (tf.math.log_sigmoid(logits2)*all_age_onehot + (tf.math.log_sigmoid(logits2) - logits2)*(1-all_age_onehot)), 1))
        all_age_loss = tf.reduce_mean(all_age_loss)

        all_logit_distribution = tf.nn.softmax(logits2, 1)
...
def cal_mae(model, images, labels):
    ...
    for i in range(137):
        ...
        age_predict2 = tf.cast(tf.less(0.5, age_predict2), tf.int32)
...
def main():
    model = GEI_age_model_V7(input_shape=(FLAGS.img_height, FLAGS.img_width, 1), weight_decay=0.00001)
